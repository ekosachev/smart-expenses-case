...
async def reg_user(login: str, password: str) -> User:
    session: AsyncSession
    async with async_session() as session:
        check = await session.execute(select(User.login).where(User.login == login))
        check = check.scalar_one_or_none()
        if check is not None:
            raise HTTPException(
                detail="login already exist", status_code=status.HTTP_400_BAD_REQUEST
            )
...
async def get_user_by_id(id_user: int):
    session: AsyncSession
    async with async_session() as session:
        user = await session.execute(select(User).where(User.id == int(id_user)))
        user = user.scalar_one_or_none()
        if user is None:
            raise HTTPException(status_code=404, detail=f"{id_user=} does not exist")
...
async def update_user(id_user: int, new_data: UserUpdate):
    session: AsyncSession
    async with async_session() as session:
        user = await session.execute(select(User).where(User.id == id_user))
        user = user.scalar_one_or_none()
        if user is None:
            raise HTTPException(
